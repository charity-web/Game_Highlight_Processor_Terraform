...
delete_ecs_services() {
    ...
    if [ -z "$SERVICES" ]; then
        ...
        for SERVICE in $SERVICES; do
            ...
            aws ecs delete-service --cluster "$CLUSTER_NAME" --service "$SERVICE" --force --region us-east-1
            echo "Waiting for service $SERVICE to be deleted..."
            aws ecs wait services-inactive --cluster "$CLUSTER_NAME" --services "$SERVICE" --region us-east-1
...
delete_ecr_repository() {
    ...
    REPO_URI=$(aws ecr describe-repositories --repository-names "$ECR_REPO_NAME" --query 'repositories[0].repositoryUri' --output text --region us-east-1)
    if [ -z "$REPO_URI" ]; then
        echo "ECR Repository $ECR_REPO_NAME does not exist."
    else
        echo "Deleting ECR Repository: $REPO_URI"
        aws ecr delete-repository --repository-name "$ECR_REPO_NAME" --force --region us-east-1
...
delete_load_balancers() {
    ...
    ALB_ARNS=$(aws elbv2 describe-load-balancers --query "LoadBalancers[?contains(Tags[].Key, 'Project') && contains(Tags[].Value, 'highlight-pipeline-final')].LoadBalancerArn" --output text --region us-east-1)
    if [ -z "$ALB_ARNS" ]; then
        echo "No ALBs found with tag prefix '$ALB_TAG_PREFIX'."
    else
        for ALB_ARN in $ALB_ARNS; do
            ALB_NAME=$(aws elbv2 describe-load-balancers --load-balancer-arns "$ALB_ARN" --query 'LoadBalancers[0].LoadBalancerName' --output text --region us-east-1)
            ...
            aws elbv2 wait load-balancer-deleted --load-balancer-arn "$ALB_ARN" --region us-east-1
...
delete_security_groups() {
    ...
    SG_IDS=$(aws ec2 describe-security-groups --filters "Name=tag:Project,Values=highlight-pipeline-final" --query 'SecurityGroups[*].GroupId' --output text --region us-east-1)
    if [ -z "$SG_IDS" ]; then
        echo "No Security Groups found with tag prefix '$SECURITY_GROUP_PREFIX'."
    else
        for SG_ID in $SG_IDS; do
            SG_NAME=$(aws ec2 describe-security-groups --group-ids "$SG_ID" --query 'SecurityGroups[0].GroupName' --output text --region us-east-1)
...
delete_log_groups() {
    ...
    LOG_GROUPS=$(aws logs describe-log-groups --log-group-name-prefix "$LOG_GROUP_PREFIX" --query 'logGroups[*].logGroupName' --output text --region us-east-1)
